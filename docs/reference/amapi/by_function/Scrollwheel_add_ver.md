...
Canonical name: scrollwheel_add_ver
Namespace: Scrollwheel
Source URL: https://wiki.siminnovations.com/index.php?title=Scrollwheel_add_ver
Revision: 4628
---
...
|---|----------|------|-------------|
| 1 | `segment_image` | String | The segment image. The scroll wheel is drawn by repeating a certain segment image. Make sure to design this segment image to fit perfectly next to each other. Note that this is both filename and extension. Only PNG files are accepted. |
...
| 9 | `direction_callback` | Function | This function will be called when the user manipulates the scroll wheel. A direction argument is supplied, where -1 is scrolled up, and 1 is scrolled down. |
...
function callback(direction)
  ...
  if direction == -1 then
    print("scroll wheel dragged up")
  end

  if direction == 1 then
    print("scroll wheel dragged down")
  end
end

-- Create a new scroll wheel
scrollwheel_id = scrollwheel_add_ver("segment.png", 0, 0, 200, 200, 50, 50, callback)
```
