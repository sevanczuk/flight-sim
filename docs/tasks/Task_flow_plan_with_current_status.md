...
**Last updated:** 2026-05-02T10:50:56-04:00 (Purple Turn 15 — C2.2-G → ✅ Done; assembly script + aggregate → ✅ Done; GNX375-PAGEMAP-PYMUPDF-01 added and ✅ Done; ITM-11 closed; "Where we are right now" updated)
...
| C2.2-F task prompt drafting                | CD drafts `docs/tasks/c22_f_prompt.md` (covers §§11–13 — Transponder + ADS-B, Audio/Alerts, Messages; ~540 lines target per D-19; most-coupled fragment; 16 hard constraints including D-16 three-modes, built-in dual-link ADS-B In, TSAA aural, Remote G3X Touch v1 out-of-scope; 25 self-review items; Coupling Summary budget ~80 lines calibrated up from ~60; §11.13 = 9 advisories total with explicit distinction from pp. 288–289 sibling-unit messages) | ✅ Done (Turn 4, 2026-04-24T06:26 ET) |
| C2.2-F (full lifecycle)                    | Execution + check-completions + compliance + archive          | ✅ **Done** (Turn 21, 2026-04-24T10:51 ET) — archived PASS WITH NOTES 36/1/1/0 across 38 checks; 606 lines (+12% — lowest relative overshoot in series); all 16 hard constraints PASS; F3 encoding issue resolved (viewer mojibake, not file corruption); ITM-12 logged (Coupling Summary format watchpoint for Fragment G) |
| C2.2-G task prompt drafting                | CD drafts `docs/tasks/c22_g_prompt.md` (covers §§14–15 + Appendix A — Persistent State, External I/O, Family Delta; ~300 lines target per D-19; smallest fragment but highest coupling footprint in Coupling Summary) | ✅ Done |
| C2.2-G (full lifecycle)                    | Final fragment lifecycle                                     | ✅ Done — archived PASS WITH NOTES 49/2/0/0 across 51; 443 lines (+48% over target); ITM-12 resolved by Coupling Summary discipline. 7-fragment decomposition complete. |
| C2.2-ASSEMBLE (full lifecycle) | `scripts/assemble_gnx375_spec.py` + aggregate spec; CD-authored prompt; CC-executed | ✅ Done — archived PASS WITH NOTES 39/3/2/1 across 45 checks (Turn 12, 2026-04-30). Outputs: 555-line script + 4433-line `GNX375_Functional_Spec_V1_aggregate.md`. ITM-13 (BOM) and ITM-14 (`--partial` continuity-skip) logged; ITM-13 closed by D-29. |
| GNX375-PAGEMAP-PYMUPDF-01 (full lifecycle) | `scripts/build_page_number_map_from_pymupdf.py` + `assets/gnx375_pymupdf_v1_0_1/page_number_map.json` (schema v2.0); built atop `pymupdf-extract V1.0.1` metadata | ✅ Done — archived ALL PASS 17/17 across 7 categories (Turn 12, 2026-05-02). 310 / 306 parsed / 4 unparseable. Includes D-28 PAP 22 override on page 2. |
| ITM-11 verification | CD-direct content-match of 4 anchor + 9 spot-check V1 citations against `assets/gnx375_llama_extract/pages/page_NNN.md` | ✅ Done — closed Turn 14, 2026-05-02. 13/13 PASS; convention captured in D-30 (V1 cites physical PDF pages, zero offset to extraction). |
| **D-24 logged** (Turn 20, 2026-04-24)       | CD logs standing rule: billable-API task prompts (LlamaParse, OpenAI, Anthropic, etc.) must enumerate all supported outputs in a dedicated section, requesting every output with a plausible downstream consumer. Prevents the Turn 18 failure mode (requested markdown-only; missed images at same credit cost). Applies to all future CD-authored CC task prompts for billable APIs. | ✅ Done (`D-24-billable-api-task-prompts-enumerate-outputs.md`) |
| **D-23 logged** (Turn 16, 2026-04-24)       | CD logs standing rule: credential-file access pattern for CC scripts — `json.load` into local var, never `cat`/`Get-Content`'d, value never printed/logged/stringified, error messages sanitized. Precedent from the LlamaParse PDF extraction task. | ✅ Done (`D-23-credential-file-access-pattern-for-cc-scripts.md`) |
| **D-22 logged** (Turn 7 initial; Turn 9 rewrite; Turn 19 §(1) amendment, 2026-04-24)       | CD logs C3 spec review customization decision, **scoped to the GNX 375 Functional Spec V1 C3 review only** (not a standing convention). Six customizations: (1) upstream PDF re-extraction via LlamaParse Agentic tier; (2) customized agent roster — `--without` 7 low-fit defaults (Implementation, API Contract, Assistant Parity, Integration, Security, Perf, Test Coverage), keep Complexity/Editorial/Enhancement/UX/Error Recovery, add 3 domain-specific agents (PDF Source Fidelity, Cross-Fragment Coupling, Sibling-Unit Consistency); (3) aggressive triage — keep CRITICALs + functional HIGHs/MEDIUMs; reject non-functional HIGHs, all LOWs, all opportunities (goal = real-device fidelity, not "better GNX 375"); (4) thorough documentation of every reject/defer (audit + deviation-investigation asset); (5) hybrid-mode + Review Priority Guide + tier escalation (V1 quick → V2 standard + `-s C` → V3 full if needed); (6) manifest pre-flight + disposition cheat-sheet as C3 tooling. Explicit scope caveat: does not apply to Design Spec D1/D2 or GNC 355 Functional Spec. Initial draft (Turn 7) over-generalized; Turn 9 rewrite corrected scope and incorporated Steve's aggressive-triage directives. | ✅ Done (`D-22-c3-spec-review-customization-for-gnx375-functional-spec.md`; old file superseded with pointer) |
| **Pre-C3 work — PDF re-extraction (text)**   | Resubmit Garmin Pilot's Guide 190-02488-01 Rev. C to LlamaParse Agentic tier. Output to `assets/gnc355_pdf_extracted/llamaparse_agentic_v1/` (D-06 gitignored). Per D-22 §(1). | ✅ Done (Turn 18, 2026-04-24T10:31 ET) — 330 pages extracted in 2.5 min; major quality uplift confirmed on §11.4 + §4.10 spot-checks. D-22 §(1) amended Turn 19 to make new extraction preferred reference for post-archive authoring. Completion report at `docs/tasks/pdf_reextraction_llamaparse_completion.md`. |
| **Pre-C3 work — PDF images (Approach A)**    | Catalog + rename the 521 PyMuPDF-extracted `.bin` files in `assets/gnc355_pdf_extracted/images/`. Detect formats via magic bytes; build JSON inventory; generate HTML gallery with markdown context captions; produce effectiveness report per criteria in briefing. Per briefing `docs/tasks/image_extraction_briefing.md`. | 🔶 **QUEUED** (Turn 20, 2026-04-24) — briefing written; CC task prompt to be drafted next. Free (no API cost). ~15–20 min CC wall-clock. Cataloging leverages existing PyMuPDF `.bin` extraction. |
| **Pre-C3 / Pre-I1–I3 work — PDF images (Approach B, conditional)** | Supplementary LlamaParse run with `output_options.images_to_save=["embedded"]` and optionally `["screenshots"]`. Leverages 48-hour cache window (expires ~2026-04-26T10:31 ET) for free-re-parse if triggered soon. Per briefing decision matrix. | ⚪ Conditional on Approach A effectiveness report. Triggered by any hard-criterion FAIL (C1/C2/C3) or 2+ soft-criterion FAILs (S1/S2/S3) per briefing. |
| **Pre-C3 work — domain-specific agents**    | Draft `.claude/agents/spec-pdf-source-fidelity-reviewer.md`, `.claude/agents/spec-cross-fragment-coupling-reviewer.md`, `.claude/agents/spec-sibling-unit-consistency-reviewer.md` (~60 lines each; Sonnet model). Target during idle windows between C2.2-F/G CC executions. Per D-22 §(2). | ⚪ Pending (per D-22)                         |
| **Pre-C3 work — assembly script**           | CD authors `scripts/assemble_gnx375_spec.py` per D-18 at C2.2-G archive step. Strips YAML/fragment headers/Coupling Summary blocks; concatenates in manifest order; adds aggregate H1. | ✅ Done (Turn 12, 2026-04-30 — archived in C2.2-ASSEMBLE row above) |
| **Pre-C3 work — manifest pre-flight**       | CD authors `scripts/verify_gnx375_manifest.py` per D-22: verifies fragments exist, line counts match, section headers continuous, no duplicate sections, Coupling Summary forward-refs resolve. | ⚪ Pending (per D-22; was "at or before C2.2-G archive" but G is archived; treat as P1 going forward) |
| **Pre-C3 work — Review Priority Guide**     | CD prepends P1/P2/P3 table to aggregate spec at C2.2-G archive step per D-22. P1: §§11, 15, 4.9, 7. P2: §§5–6, 8–10, 14, 12, 13. P3: §§1–4, Appendices. | ⚪ Pending (treat as P1 alongside manifest pre-flight)                 |
| Aggregate spec                             | Assembly script (`scripts/assemble_gnx375_spec.py`) + single-file aggregate; CD authors at C2.2-G archive per D-18 | ✅ Done (Turn 12, 2026-04-30 — 4433-line aggregate at `docs/specs/GNX375_Functional_Spec_V1_aggregate.md`) |
...
**Updated 2026-05-02T10:50:56-04:00 (Purple Turn 15).** Older snapshot (C2.2-E archive 2026-04-23) preserved below for historical reference.

**Stream C status:** All 7 V1 fragments archived; assembly script + aggregate (`GNX375_Functional_Spec_V1_aggregate.md`, 4433 lines) done at C2.2-ASSEMBLE (Turn 12, 2026-04-30). Recent additional work:

- **GNX375-PAGEMAP-PYMUPDF-01 archived (Turn 12, 2026-05-02)** — 17/17 ALL PASS. Built `scripts/build_page_number_map_from_pymupdf.py` (~370 lines) atop `pymupdf-extract V1.0.1` metadata. Output: `assets/gnx375_pymupdf_v1_0_1/page_number_map.json` schema v2.0 with 310 / 306 parsed / 4 unparseable.
- **ITM-11 closed (Turn 14, 2026-05-02)** — 13/13 V1 citation verification PASS by direct content match against `assets/gnx375_llama_extract/pages/page_NNN.md`. ITM-11's offset claim was a misdiagnosis. Convention captured in D-30 (V1 cites physical PDF pages, zero offset to extraction).
- **ITM-13 closed (Turn 11, 2026-05-02)** by D-29 — D-29 supersedes D-04 with simple `git commit -m "... [AI commit]" -m "..."` format; the file-based commit pattern that produced the BOM is gone, so the failure mode is unreachable.
- **D-28 logged (Turn 3, 2026-05-02)** — page 2 of the GNX 375 Pilot's Guide PDF carries a paper-recycling identifier ("22 PAP") that the parser misread as a page number; manual override + override-file mechanism in the page-map transform.
- **D-29 logged (Turn 11, 2026-05-02)** — commit policy simplification.
- **D-30 logged (Turn 14, 2026-05-02)** — V1 fragment citation convention.

**Pre-C3 work remaining (P1 pending per `priority_task_list.md`):**

1. `scripts/verify_gnx375_manifest.py` authoring (per D-22 §6)
2. Review Priority Guide prepend to aggregate (per D-22 §5)
3. Three domain-specific Sonnet agents (`.claude/agents/spec-pdf-source-fidelity-reviewer.md`, `spec-cross-fragment-coupling-reviewer.md`, `spec-sibling-unit-consistency-reviewer.md`) per D-22 §2
4. **DEPENDENCY-AUDIT-01** — prompt drafting in progress at Purple Turn 15. CD-authored prompt; CC-executable. Inventory of references to retired asset paths across the repo.

**After P1 work clears:** C3 `/spec-review` V1 (quick tier per D-22 customizations).

**Open ITMs (low severity):** ITM-02, ITM-03, ITM-04, ITM-05, ITM-08 (residual), ITM-10 (watchpoint), ITM-14 (defer). FE-01 (defer).

---

### Where we were at C2.2-E archive (Turn 3 post-session-reset, 2026-04-23 — historical snapshot)
...
**C2.2-F task prompt drafting complete (Turn 4, 2026-04-24T06:26 ET).** Scope: §§11–13 (Transponder + ADS-B, Audio/Alerts/Annunciators, Messages). Target ~540 lines per D-19; expected actual ~600–700 given §11's 14 sub-sections + §13's 13 sub-sections + dense cross-references. **16 hard constraints** (the most of any fragment): ITM-08 grep-verify; Coupling Summary ~80-line budget (calibrated up from ~60); no §4 / §4.9 / §10.12 re-authoring; D-16 three-modes only (§11.4); D-16 Remote G3X Touch v1 out-of-scope (§11.10); D-16 built-in dual-link ADS-B In (§11.11); §11.13 = exactly 9 advisories (4 from pp. 283–284 + 5 from p. 290); §13.9 and §13.11 cross-ref §11.13 (no re-enumeration); §13.11 explicit distinction from pp. 288–289 sibling-unit messages; D-16 TSAA aural on GNX 375 (§12.4); §12.4 OPEN QUESTION 6 cross-ref only (no verbatim re-preservation; Fragment C §4.9 is authoritative); §12.9 XPDR Annunciations wholesale replaces COM Annunciations; §13.9 XPDR Advisories wholesale replaces COM Radio Advisories; no §§1–10/14–15/Appendix headers; no COM present-tense on GNX 375; external altitude source framing consistent across §11.1/11.3/11.4/11.8/11.13. **25 self-review items.** Open questions preserved: OPEN QUESTION 4 (XPL XPDR dataref names), OPEN QUESTION 5 (MSFS SimConnect variables), OPEN QUESTION 6 cross-ref only to §4.9, §11.10 v1 out-of-scope flag, §11.11 simulator-availability flag.

Ready to launch CC with:
```
Read and execute docs/tasks/c22_f_prompt.md
```

**D-22 logged (Turn 7 initial; Turn 9 rewrite, 2026-04-24T07:18 ET):** C3 spec review customization **for the GNX 375 Functional Spec V1 C3 review cycle only** (explicit scope caveat: does not apply to future specs; later specs should re-derive customizations from their own success criteria). Six customizations: (1) upstream PDF re-extraction via LlamaParse Agentic tier (improves source-of-truth directly, not just the agent checking against it); (2) customized agent roster — `--without` 7 low-fit defaults, keep 5 relevant defaults, add 3 domain-specific agents for PDF fidelity + cross-fragment coupling + sibling-unit consistency; (3) **aggressive triage stance** — keep CRITICALs and only those HIGH/MEDIUM findings that could lead to functional errors; reject HIGH/MEDIUM/LOW findings that are editorial/stylistic; be wary of opportunities (goal is real-device fidelity, not "better GNX 375"); (4) **thorough documentation of every rejected/deferred finding** — serves as audit trail AND as a deviation-investigation asset for I1–I3 implementation phase; (5) hybrid-mode review + Review Priority Guide + tier escalation (V1 quick → V2 standard + `-s C` → V3 full only if needed); (6) manifest pre-flight script + disposition cheat-sheet as C3 tooling. Initial Turn 7 draft over-generalized the triage stance into a standing convention; Turn 9 rewrite corrected scope per Steve's explicit directive and added the LlamaParse upstream step and triage criteria. Old D-22 file preserved as a superseded pointer for audit trail.

**Critical path (unchanged):** ~6 more sequential CD steps to C3 kick-off:
