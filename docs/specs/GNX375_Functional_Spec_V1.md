...
**Status:** Draft (7 of 7 fragments authored — **assembly-ready**)
...
| 6 | C2.2-F | `fragments/GNX375_Functional_Spec_V1_part_F.md` | §§11–13 (Transponder + ADS-B, Audio/Alerts, Messages) | ✅ Archived (Turn 21, 2026-04-24) | 540 / 606 |
| 7 | C2.2-G | `fragments/GNX375_Functional_Spec_V1_part_G.md` | §§14–15 + Appendix A (Persistent State, External I/O, Family Delta) | ✅ Archived (Turn 6, 2026-04-25) | 300 / 443 |
...
**Overage trend (7 of 7 fragments archived):** A=+22%, B=+11%, C=+26%, D=+22%, E=+82%, F=+12%, G=+8% (vs. prompt-adjusted ~410 target). Fragment G is the **lowest absolute overshoot in the series** — §15 dataref tables added rows beyond outline estimate, but Coupling Summary stayed at 105 lines (within the ITM-12 95–105 target). Pattern across the full series: structural overage (sub-section count, table rows) drove every fragment's growth; no padding observed.
...
| 2026-04-24 | Fragment F drafted, executed (606 lines; +12% overage — **lowest relative overshoot in the series**), Phase 1 + Phase 2 compliance **PASS WITH NOTES (36 PASS / 1 PASS WITH NOTES / 1 PARTIAL / 0 FAIL across 38 checks)**; archived. Notable outcomes: (a) all 16 hard constraints PASS; (b) F3 encoding concern raised in Phase 1 resolved — `��` observation was viewer mojibake (Windows-1252 reading UTF-8); actual file has 169 valid § characters and 0 U+FFFD replacement characters; (c) S6 nine-advisory verbatim match — all 4 XPDR/ADS-B Out advisories from pp. 283–284 and all 5 GNX 375 Traffic advisories from p. 290 match PDF character-by-character; pp. 288–289 (GDL 88 LRU) set explicitly excluded per D-16; (d) C1 ITM-08 grep-verify — all 20 claimed Appendix B terms at exact line numbers (± 0); 5 exclusions confirmed absent from Appendix B; (e) D-16 framing fully honored — three XPDR modes only, WOW automatic, built-in dual-link ADS-B In, TSAA aural GNX-375-only, Remote G3X Touch v1 out-of-scope, ADS-B traffic log GNX-375-only; (f) one PARTIAL on C2 Coupling Summary line count — 39 lines vs. ~80 target (content complete: 14 backward-refs + 5 forward-refs + 4 intra-fragment + outline footprint; compact bullet format vs. expanded prose) — logged as ITM-12 watchpoint for Fragment G. |
| 2026-04-25 | Fragment G drafted, executed (443 lines; +8% overage — **lowest absolute overshoot in the series**), Phase 1 + Phase 2 compliance **PASS WITH NOTES (49 PASS / 2 PASS WITH NOTES / 0 PARTIAL / 0 FAIL across 51 checks)**; archived. **7-fragment decomposition complete; spec body assembly-ready.** Notable outcomes: (a) all 17 hard constraints PASS; (b) ITM-12 discipline successfully applied — Coupling Summary 105 lines (within 95–105 target) in prose-per-ref format with 14 backward-ref blocks at 2–4 sentences each; (c) ITM-08 discipline maintained — 27 Appendix B terms grep-verified present, 5 exclusions (EPU, HFOM, VFOM, HDOP, TSO-C151c) confirmed absent; (d) D-15 framing explicit in §15.6 (no on-screen VDI; vertical deviation output-only to external VDI for LPV/LP+V/LNAV+V; design-phase flag for output dataref names); (e) D-16 framing explicit in §14.1 (three modes only: Standby/On/Altitude Reporting) and §15.7 (external ADC/ADAHRS altitude source; advisory trigger cross-refs §11.13/§13.9); (f) D-12 framing explicit in Appendix A.1 (GNX 375 primary; D-02 GNC 375 nomenclature error resolved; GNC 355 deferred); (g) OQ4 (XPL XPDR datarefs) and OQ5 (MSFS XPDR SimConnect variables) preserved verbatim for D1 Design Spec resolution; (h) two PASS WITH NOTES are completion-report self-count discrepancies (S12 cosmetic bracket formatting on `p. 155` citation; C12 GNC 355 Adds list count is 10 not 9 — all 10 items legitimate GNC 355 features) — spec content correct in both cases; (i) all 13 forward-refs from Fragments A–F to Fragment G content resolved; (j) no duplicate H2 headings across all 7 fragments (other than expected `## Coupling Summary` which is stripped on assembly). Next phase: V1 aggregate assembly via `scripts/assemble_gnx375_spec.py` (C2.2-ASSEMBLE prompt drafted at `docs/tasks/c22_assemble_gnx375_prompt.md`; gated on this archive). |
