...
function new_oil_fsx(bus_volts, pressfsx, tempfsx, tempa2a, pressa2a)
    ...
    if tempa2a ~= nil and tempa2a ~= 0 then
        temp = tempa2a * 1.8 + 32
    else
        temp = tempfsx
    end
    ...
    if pressa2a ~= nil and pressa2a ~= 0 then
        press = pressa2a
    else
        press = pressfsx
    end
...
function timer_callback()    
    ...
    if cur_oilt > 200 then
        rotate(img_oilt, 63 - (25 / 45 * (cur_oilt - 200)) )
    elseif cur_oilt > 100 then
        rotate(img_oilt, 128 - (65 / 100 * (cur_oilt - 100)) )
    else
        rotate(img_oilt, 139 - (11 / 25 * (cur_oilt - 75)) )
    end

    rotate(img_oilp, -139 + (cur_oilp * 0.9) )
