...
--   Init                                  --
---------------------------------------------
local flag_rotation   = -25
rotate(img_flag, flag_rotation) -- failed on startup until SIM engages
local flag_state      = 0       -- 0 = do nothing, 1 = extend, 2 = retract
local flag_state_prev = 0
...
function new_rotation(rotation)
    rotate(img_background_compass, rotation * -1)
end

function new_headbug(heading, bug)
    rotate(img_bug, bug - heading)
end
...
--   Timer start                           --
---------------------------------------------
tmr_flag = timer_start(0, 10, animation_timer_callback)
